...
class SubprojectCreation(models.TransientModel):
    ...
    land_sale = fields.Boolean('Land Sale')

    @api.model
    def default_get(self, fields):
        res = super(SubprojectCreation, self).default_get(fields)
        active_id = self._context.get("active_id", False)
        project_id = self.env["property.project"].browse(active_id)
        if project_id.property_type == 'land' and project_id.project_for == 'sale':
                res['land_sale'] = True
        return res
    ...
    def create_sub_project(self):
        ...
        data = {
            ...
            "zip": project_id.zip,
            "total_floors": self.floors if self.floors else 0,
            "units_per_floor": self.units_per_floor if self.units_per_floor else 0,
            "total_area": self.total_area if self.total_area else 0,
            "available_area": self.total_area if self.total_area else 0,
            "land_sale": True if self.total_area > 1 else False,
