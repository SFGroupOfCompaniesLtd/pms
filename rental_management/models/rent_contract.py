...
class TenancyDetails(models.Model):
    ...
    total_bill_amount = fields.Monetary(
        string='Bill', compute="_compute_total_bill_amount")
    ...
    def write(self, vals):
        if "payment_term" in vals:
            ...
            if rent_unit in valid_payment_terms and payment_term not in [term.lower().replace(" ", "_") for term in valid_payment_terms[rent_unit]]:
                raise ValidationError(
    ...
    # Compute
    # Contract End Date
    @api.depends('start_date', 'month', 'rent_unit', 'payment_term', 'duration_type', 'duration_end_date')
                ...
                if today_date < rec.end_date:
                    ...
                    if today_date == next_invoice_date:
                        ...
                        if rec.is_extra_service and rec.extra_service_invoice == 'merge':
                            for line in rec.extra_services_ids.filtered(lambda line: line.service_type == 'monthly'):
