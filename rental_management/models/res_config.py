...
class RentalConfig(models.TransientModel):
    ...
    broker_item_id = fields.Many2one('product.product', string="Broker Commission Item",
                                     default=lambda self: self.env.ref(
                                         'rental_management.property_product_3', raise_if_not_found=False),
    ...
    gva_url = fields.Char(string='GVA URL',config_parameter='rental_management.gva_url')
    gva_access_token = fields.Char(string='GVA Access Token',config_parameter='rental_management.gva_access_token')
    malipopay = fields.Boolean(string='Malipopay',config_parameter='rental_management.malipopay', default=False)

    booking_due_days = fields.Integer(string="Booking Due days", config_parameter='rental_management.booking_due_days')
    full_payment_due_days = fields.Integer(string="Full Payment Due days", config_parameter='rental_management.full_payment_due_days')
