...
class TenancyInvoice(models.Model):
    ...
    sale_invoice_id = fields.Many2one(
        'sale.invoice', string="Sale Invoice.")

    invoice_seq = fields.Char('Invoice Sequence')
    control_number = fields.Char('Control Number',readonly=True)
    booking_invoice = fields.Boolean('Booking Invoice')
    defaulter_move = fields.Boolean('Defaulter Move')

    def _post(self, soft=True):
        to_post = super()._post(soft)
        if to_post.sale_invoice_id:
            if to_post.sale_invoice_id.payment_type == 'full_payment':
                to_post.invoice_seq = to_post.name
            if to_post.sale_invoice_id.payment_type == 'quarterly':
                account_move = self.env['account.move'].search(
                    [('sold_id', '=', to_post.sale_invoice_id.property_sold_id.id), ('booking_invoice', '=', False)],
                    order="id", limit=1)
                if account_move:
                    to_post.invoice_seq = account_move.name + '-' + 'Q'
                else:
                    to_post.invoice_seq = to_post.name + '-' + 'Q'
            if to_post.sale_invoice_id.payment_type == 'monthly':
                account_move = self.env['account.move'].search(
                    [('sold_id', '=', to_post.sale_invoice_id.property_sold_id.id), ('booking_invoice', '=', False)],
                    order="id", limit=1)
                if account_move:
                    to_post.invoice_seq = account_move.name + '-' + 'M'
                else:
                    to_post.invoice_seq = to_post.name + '-' + 'M'
        return to_post

    def account_payment_check(self):
        move_ids = self.env['account.move'].search(
            [('state', '=', 'posted'), ('status_in_payment', 'not in', ['paid', 'cancel'])])
        today_date = fields.Date.today()
        for move in move_ids:
            if today_date > move.invoice_date_due:
                move.partner_id.defaulter = True
                move.defaulter_move = True

    def invoice_payment_reminder(self):
        move_ids = self.env['account.move'].search(
            [('state', '=', 'posted'), ('status_in_payment', 'not in', ['paid', 'cancel'])])
        for move in move_ids:
            remainder_date = move.invoice_date_due + relativedelta(days=-1)
            if remainder_date == fields.date.today():
                mail_template = self.env.ref(
                    'rental_management.sale_invoice_payment_mail_template_reminder', raise_if_not_found=False)
                if mail_template:
