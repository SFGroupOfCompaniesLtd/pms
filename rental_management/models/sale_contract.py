...
class PropertyVendor(models.Model):
    ...
    stage = fields.Selection([('booked', 'Booked'), ('refund', 'Refund'), ('sold', 'Sold'), ('cancel', 'Cancel'), ('locked', 'Locked')], string='Stage')
    ...
    gva_reference = fields.Char('Book GVA Reference',readonly=True)
    gva_payment_id = fields.Char('Book GVA Payment',readonly=True)
    sold_gva_reference = fields.Char('Sold GVA Reference',readonly=True)
    sold_gva_payment_id = fields.Char('Sold GVA Payment',readonly=True)
    ...
    def _compute_maintenance_request_count(self):
        for rec in self:
            request_count = self.env['maintenance.request'].search_count([('sell_contract_id', 'in', [rec.id])])
    ...
    def action_create_invoices(self):
        ...
        for sale_in in self.sale_invoice_ids:
            sale_in.action_create_invoice()
        for sale_in in self.sale_invoice_ids:
            inv_ref = sale_in.invoice_id.invoice_seq

        main_data = {'reference': inv_ref,
         "amount": self.total_sell_amount,
         'customer': {'name': self.customer_id.name, 'email': self.customer_email, 'phoneNumber': int(self.customer_phone.replace(' ','').replace('+','') or self.customer_id.mobile.replace(' ','').replace('+','')),},
         'land_details': {'plot_number': self.property_id.name, 'location': self.region_id.name if self.region_id else '', 'total_price': self.total_sell_amount,
                          'booking_amount': self.book_price * -1, 'currency': 'USD'},}

        for sale_in in self.sale_invoice_ids:
            installment_dict = {
                "installment_no": sale_in.invoice_id.name,
                "amount": sale_in.invoice_id.amount_total,
                "due_date": str(sale_in.invoice_id.invoice_date_due),
                "status": "PENDING"
            }
            instalments.append(installment_dict)
        main_data['installments'] = instalments
        ...
        if malipopay:
            ...
            user_response_data = requests.post(
                gva_url, headers=headers, data=json.dumps(main_data))

            return_data = user_response_data.json()
            sold_gva_reference = return_data['data'].get('reference')
            sold_gva_payment_id = return_data['data'].get('payment_id')
            self.sold_gva_reference = sold_gva_reference
            self.sold_gva_payment_id = sold_gva_payment_id
            for sale_in in self.sale_invoice_ids:
                sale_in.invoice_id.control_number = sold_gva_payment_id

    def check_booking_invoice(self):
        vendor_invoices = self.env['property.vendor'].search([])
        for vendor in vendor_invoices:
            if vendor.book_invoice_id and vendor.property_id.stage == 'booked':
                remainder_date = vendor.book_invoice_id.invoice_date_due + relativedelta(days=-1)
                if remainder_date == fields.date.today():
                    mail_template = self.env.ref(
                        'rental_management.property_book_mail_template_reminder', raise_if_not_found=False)
                    if mail_template:
...
class SaleInvoice(models.Model):
    ...
    payment_type = fields.Selection(
        [('monthly', 'Monthly'), ('quarterly', 'Quarterly'), ('full_payment', 'Full Payment')], string="Payment Type",
    ...
    def action_create_invoice(self):
        ...
        full_payment_due_days = self.env['ir.config_parameter'].sudo().get_param(
            'rental_management.full_payment_due_days')
        gva_url = self.env['ir.config_parameter'].sudo().get_param('rental_management.gva_url')
        gva_access_token = self.env['ir.config_parameter'].sudo().get_param('rental_management.gva_access_token')
        malipopay = self.env['ir.config_parameter'].sudo().get_param('rental_management.malipopay')
        invoice_id = self.env['account.move'].sudo().create({
            ...
            'invoice_date': fields.date.today() if self.payment_type == 'full_payment' else self.invoice_date,
            'invoice_date_due': fields.date.today() + relativedelta(days=int(full_payment_due_days)) if self.payment_type == 'full_payment' else self.invoice_date,
        ...
        # self.invoice_id.sale_invoice_id = self.id
        self.invoice_created = True
        self.action_send_sale_invoice(invoice_id.id)
        if invoice_id.state == 'posted' and self.payment_type == 'full_payment' and malipopay:
            data = {
                "reference": invoice_id.name,
                "description": 'Land purchase Full payment of ' + self.property_sold_id.property_id.name,
                "channel": "Bank Transfer",
                "amount": invoice_id.amount_total,
                "sold_invoice_id":invoice_id.id,
                "customer": {
                    "name": self.property_sold_id.customer_id.name,
                    "email": self.property_sold_id.customer_id.email,
                    "phoneNumber": self.property_sold_id.customer_id.phone.replace(' ', '').replace('+',
                                                                                   '') or self.customer_id.mobile.replace(
                        ' ', '').replace('+', '')
                },
                "land_details": {
                    "plot_number": self.property_sold_id.property_id.name,
                    "location": self.property_sold_id.region_id.name if self.property_sold_id.region_id else '',
                    "total_price": invoice_id.amount_total,
                    "booking_amount": self.property_sold_id.book_price * -1,
                    "currency": "USD"
                }
            }
            headers = {"Authorization": gva_access_token,
            ...
            sold_gva_reference = return_data['data'].get('reference')
            sold_gva_payment_id = return_data['data'].get('payment_id')
            self.property_sold_id.sold_gva_reference = sold_gva_reference
            self.property_sold_id.sold_gva_payment_id = sold_gva_payment_id
            invoice_id.control_number = sold_gva_payment_id
