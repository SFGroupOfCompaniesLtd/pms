...
class RentalPortalWebsite(http.Controller):
    ...
    @http.route(['/my/maintenance-request/information/<model("maintenance.request"):mr>'], type='http', auth="user",
                website=True)
...
class WebhookPaymentData(http.Controller):
    @http.route("/payment/info", auth="public", type="json", methods=['POST', 'GET'])
    def payment_info(self, **post):
        response = request.get_json_data()
        _logger.info('\n\nPayment Response\n\n %s',
                     response)
        if response:
            if response.get('sold_invoice_id') and response.get('reference'):
                move_id = request.env['account.move'].sudo().search(
                    [('id', '=', response.get('sold_invoice_id'))])
                date_string = response.get('date')
                date_object = datetime.strptime(date_string, "%d-%m-%Y").date()
                _logger.info('\n\nPayment Detail\n\n %s%s%s',
                             date_object, response.get('amount'), move_id)
                vals = {
                    'move_id':move_id.id,
                    'payment_date':date_object,
                    'amount': response.get('amount')
                }
                pay_history = request.env['pay.history'].sudo().create(vals)
            if response.get('reference') and not response.get('sold_invoice_id'):
                move_id = request.env['account.move'].sudo().search([('name', '=', response.get('reference'))])
                date_string = response.get('date')
                date_object = datetime.strptime(date_string, "%d-%m-%Y").date()
                _logger.info('\n\nPayment Detail\n\n %s%s%s',
                             date_object, response.get('amount'), move_id)
